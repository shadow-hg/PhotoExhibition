...
import { usePhotos, useAdminCreatePhoto, useAdminDeletePhoto } from '../api/hooks';
import { PhotoPayload } from '../types/api';
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[2fr,1fr]">
      <div>
        <div className="flex items-end justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">当前共有 {photos?.length ?? 0} 幅作品，精选 {featuredCount} 幅。</p>
          </div>
        </div>
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          {photos?.map((photo) => (
            <article key={photo.id} className="overflow-hidden rounded-3xl border border-white/10 bg-slate-900/50 shadow-aurora">
              ...
              <div className="space-y-3 p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {photo.tags.map((tag) => (
                    <span key={tag} className="rounded-full bg-white/10 px-2 py-1 text-[10px] uppercase tracking-widest text-slate-200">
                      #{tag}
                    </span>
                  ))}
                </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-slate-900/60 p-6">
        ...
        <p className="mt-1 text-sm text-slate-400">填写作品核心信息，支持自动同步到前台展示。</p>
